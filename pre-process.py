...
svi_dir = "geocav_proj/SVI_data"



def add_SVI(svi_dir, data_dir):
    '''
    Adds SVI to factor data
    '''

    svi_files = [f for f in os.listdir(svi_dir)]
    # Arrange into SVI_Score.csv: StateFIPS,State,CountyFIPS,County,Year,Value
    # 1,Alabama,1001,Autauga,2016.0,14.93
    ...
    for file in svi_files:
        ...
        # Example Title: SVI_2010_US_county.csv
        # Example Entry: 01,ALABAMA,AL,Autauga,01001,"Autauga County, Alabama"
    ...
    # Ouput SVI data to CDC Data/factors/SVI_Score.csv
    svi_output_path = os.path.join(data_dir, "factors", "County_Level", "SVI_Score.csv")
    os.makedirs(os.path.dirname(svi_output_path), exist_ok=True)
    svi_data.to_csv(svi_output_path, index=False)
    print(f"Saved SVI data to {svi_output_path}")

def preprocess_CDC_data(data_dir, output_dir, periods):
    output_data = {}
