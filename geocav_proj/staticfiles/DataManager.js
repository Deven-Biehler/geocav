...
export class DataManager {
    constructor() {
        this.cancer_data = null; // Cache for cancer data
        this.factor_data = null; // Cache for factor data
        this.cancer_years = {}; // Cache for available years for each cancer type and level
        this.factor_years = {}; // Cache for available years for each factor and level

        // Fetch static info on initialization such as available years for cancer types and factors
        this.readyPromise = this.fetchstaticInfo().catch(err => {
            console.error('[DataManager] Error fetching static info:', err);
        });
    }

    async fetchstaticInfo() {
        console.log('[DataManager] Fetching static info for available years and types...');
        // Fetches static information such as available years, cancer types, and factors for dropdowns
        const response = await fetch('/get_static_info');
        const data = await response.json();
        this.cancer_years = data.cancer_types;
        this.factor_years = data.factor_types;
        console.log('[DataManager] Static info fetched:', { cancer_years: this.cancer_years, factor_years: this.factor_years });
    }
